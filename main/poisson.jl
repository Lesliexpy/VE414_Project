...
treeFileName = "./csv/tree_position.csv"
projFileName = "../data/data_proj_414.csv"
...
farFile = CSV.read("./csv/far_map2.csv");
closeFile = CSV.read("./csv/close_map2.csv");
...
save_array("./csv/far_map_linear.csv", dnd)
save_array("./csv/close_map_linear.csv", dnd)
...
dnf = DataFrame(X = org_x, Y = org_y);

save_array("./csv/tree_position_overall.csv", dnf)

pw_far = autoFar(projFileName,"Jiuling in observed area")
size_x = 5
...
pw_far2 = autoFar(projFileName,"Jiuling in unobserved area")
size_x = 5
